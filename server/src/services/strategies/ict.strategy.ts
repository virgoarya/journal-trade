...
class ICTStrategy {
  ...
  private buildICTChecklist(
      ...
      return evaluateWaterfall([
        ...
        },
        {
          ...
          timeframe: entryTfLabel,
          condition: this.confirmC2DNTLTF(isBuy, fractal),
          isIndependent: true,
        },
        {
          id: "ict-smt",
  ...
  private buildICTDisplayItems(sig: ICTSignal, killzone: KillzoneType, fractal?: import("./market-structure.service").FractalContext): ChecklistItem[] {
      ...
      return [
              ...
              mk("ict-bos", `② HTF Structure (${htfTfLabel}) : ${isHtfDirectional ? (isBuy ? "Bullish BOS" : "Bearish BOS") : "Unconfirmed"}`, !!isHtfDirectional, { isFailable: true, isIndependent: true, value: isHtfDirectional ? "Aligned" : "Misaligned", details: "Trend HTF harus searah signal.", timeframe: htfTfLabel }),
              mk("ict-sweep", (hasSweep || hasIFVG || isC2Closure || isC2DNT || isDCM || isSMR) ? `③ Liquidity Sweep / Inducement (${setupTfLabel})` : `③ Liquidity Sweep Menunggu @ ${targetSweep}`, hasSweep || hasIFVG || isC2Closure || isC2DNT || isDCM || isSMR, { isFailable: true, isIndependent: true, value: (hasSweep || hasIFVG || isC2Closure || isC2DNT || isDCM || isSMR) ? "Valid" : `Target @ ${targetSweep}`, details: "Swing liquidity disapu + inducement confirmation.", timeframe: setupTfLabel }),
              ...
              mk("ict-rr", `⑧ Risk-to-Reward 1:2 ${isRRValid ? "terpenuhi" : "belum"}`, hasPattern && isRRValid, { isFailable: true, isIndependent: true, details: isRRValid ? `R:R 1:${rrRatio.toFixed(2)} | SL ${sig.sl.toFixed(5)} | TP ${sig.tp.toFixed(5)}` : "RR < 1:2, signal di-drop engine." }),
            ];
    }
  ...
  private confirmC2DNTLTF(isBuy: boolean, fractal?: import("./market-structure.service").FractalContext): boolean {
    ...
    const isValid = isBuy ? (last.close > last.open && last.close > prev.high) : (last.close < last.open && last.close < prev.low);
