# System Monitoring Report

**Generated:** 19/7/2026, 22.43.04 WIB
**Status:** 🔴 CRITICAL

---

## 🤖 AI Insights (9Router)

MT5 koneksi mati. Pipeline auto-pause. Cek broker login, network, terminal crash.

Memory 96%. 5.93GB total. 57.5 jam uptime. Restart sistem segera. Memory leak atau swap habis.

CPU 4 core. Memory 96% artinya bottleneck RAM, bukan CPU. Tambah RAM atau kurangi pipeline.

Data issues 3. Pipeline cuma 1. Data corrupt atau feed gagal. Cek log pipeline.

Circuit state CLOSED. Artinya breaker belum trip. Tapi MT5 mati, memory mau penuh. Breaker akan trip sendiri. Restart dulu.

---

## Executive Summary

2 critical issue(s), 10 warning(s)

## 🔴 Critical Issues

- [mt5] MT5 connection lost
-   -> MT5 tidak terhubung - pipeline auto-paused

## 🟡 Warnings

- [system_resources] High memory usage
-   -> CPU: 4 cores
-   -> Memory: 96% used (5.93GB total)
-   -> Uptime: 57.5h
-   -> Platform: win32 10.0.19045
-   -> ⚠️ Memory usage tinggi: 96.0% (threshold: 85%)
- [data_quality] Data quality issues detected (3)
-   -> [XAUUSD.l] MT5 not connected - cannot query rates
-   -> [BTCUSD.l] MT5 not connected - cannot query rates
-   -> [NAS100.l] MT5 not connected - cannot query rates

## Recommended Actions

| # | Action | Component |
|---|--------|-----------|
| 1 | Restart MT5 MCP service | - |
| 2 | Verify MT5 terminal is running on broker VPS | - |
| 3 | Check MT5 MCP server logs in logs/mt5-errors.log | - |

## Metrics Dashboard

| Metric | Value |
|--------|-------|
| connected | 0 |
| circuitState | CLOSED |
| activePipelines | 1 |
| 6a26146a9cad211ba0631027.running | 1 |
| 6a26146a9cad211ba0631027.paused | 0 |
| 6a26146a9cad211ba0631027.error | 0 |
| 6a26146a9cad211ba0631027.totalTrades | 0 |
| 6a26146a9cad211ba0631027.pnl | 0 |
| 6a26146a9cad211ba0631027.errors1h | 0 |
| totalProviders | 6 |
| activeProviders | 6 |
| hibernasiProviders | 0 |
| circuitOpenProviders | 0 |
| provider.deepseek | active |
| provider.gpt | active |
| provider.gemini | active |
| provider.mistral | active |
| provider.nemotron | active |
| provider.claude-opus | active |
| availableForConsensus | 6 |
| cpuCount | 4 |
| memoryTotalGB | 5.93 |
| memoryUsagePercent | 96 |
| uptimeHours | 57.5 |
| checkedPipelines | 1 |
| dataIssues | 3 |

## Detailed Health Checks

### 🔴 MT5

- **Severity:** critical
- **Summary:** MT5 connection lost
- **Time:** 2026-07-19T15:43:04.530Z

- MT5 tidak terhubung - pipeline auto-paused

### 🟢 PIPELINE

- **Severity:** healthy
- **Summary:** 1 pipeline(s) running
- **Time:** 2026-07-19T15:43:04.715Z

- Found 1 active pipeline(s)

### 🟢 LLM_CONSENSUS

- **Severity:** healthy
- **Summary:** 6 provider(s) available for consensus
- **Time:** 2026-07-19T15:43:04.531Z

- ✅ DeepSeek V4 (deepseek): active
- ✅ GPT OSS 120B (gpt): active
- ✅ Gemini 2.5 Flash (gemini): active
- ✅ Mistral Large (mistral): active
- ✅ Nemotron 3 Ultra (nemotron): active
- ✅ Claude Opus 4.7 (claude-opus): active

### 🟡 SYSTEM_RESOURCES

- **Severity:** warning
- **Summary:** High memory usage
- **Time:** 2026-07-19T15:43:04.531Z

- CPU: 4 cores
- Memory: 96% used (5.93GB total)
- Uptime: 57.5h
- Platform: win32 10.0.19045
- ⚠️ Memory usage tinggi: 96.0% (threshold: 85%)

### 🟡 DATA_QUALITY

- **Severity:** warning
- **Summary:** Data quality issues detected (3)
- **Time:** 2026-07-19T15:43:04.715Z

- [XAUUSD.l] MT5 not connected - cannot query rates
- [BTCUSD.l] MT5 not connected - cannot query rates
- [NAS100.l] MT5 not connected - cannot query rates

---

*Report generated by SystemMonitorAgent v1.0 | Next report: 19/7/2026, 23.43.19 WIB*
