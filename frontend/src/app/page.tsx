  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden"
      ...
      <main className="relative z-10 flex flex-col items-center w-full max-w-[420px] px-10 py-12 text-center animate-in fade-in zoom-in-95 duration-1000"
        ...
        {/* Login Button — vanilla JS fallback, no React hydration needed */}
        <button 
          ...
          style={{ backgroundColor: '#5865F2', color: '#FFFFFF', border: 'none' }}
        >
      ...
      {/* Vanilla JS login handler — works without React hydration */}
      <script dangerouslySetInnerHTML={{ __html: `
        document.getElementById('discord-login-btn').addEventListener('click', function() {
          ...
          fetch('/api/auth/sign-in/social', {
            ...
            body: JSON.stringify({ provider: 'discord', callbackURL: window.location.origin + '/dashboard' }),
            credentials: 'include'
          })
        ...
        console.log('[AUTH] Vanilla JS login handler attached.');
      `}} />
    </div>
  );
}