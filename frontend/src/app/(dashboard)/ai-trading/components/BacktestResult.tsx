...
function MetricCard({ icon: Icon, label, value, color = "text-text-primary", sub }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 relative z-10">
        ...
        <MetricCard icon={Percent} label="Sharpe" value={(result.sharpeRatio || 0).toFixed(2)} color={(result.sharpeRatio || 0) >= 1 ? "text-green-400" : "text-text-secondary"} />
      </div>
                ...
                {result.symbolStats.map((stat, i) => (
                  <tr key={i} className="hover:bg-surface/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center font-mono text-xs">
                      ...
                      {(stat as any).breakEvenTrades > 0 && <span className="text-text-muted"> / {(stat as any).breakEvenTrades}BE</span>}
                    </td>
              ...
              {(result.methodologyStats || []).map((meth, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-surface/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-mono text-text-muted">{meth.avgConfidence}%</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {analysis && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
          className="pt-6 border-t border-border-subtle relative z-10">
          ...
          {analysis.methodologyRecommendations && analysis.methodologyRecommendations.length > 0 && (
            <div className="mb-4 p-4 bg-surface/80 border border-border-subtle/80 rounded-xl">
              <p className="text-xs uppercase tracking-wider text-text-muted font-semibold mb-3 flex items-center gap-2"><Layers className="w-3 h-3" /> Methodology Recommendations</p>
              {analysis.methodologyRecommendations.map((r, i) => {
                ...
                return (<div key={i} className="flex items-start gap-2 text-sm text-text-secondary mb-2">
          ...
          <div className="p-5 bg-surface/80 border border-border-subtle/80 rounded-xl">
            ...
            {analysis.strengths?.length > 0 && (<div className="mt-4 space-y-1">
              {analysis.strengths.map((s, i) => <div key={i} className="flex items-start gap-2 text-sm"><CheckCircle2 className="w-4 h-4 text-green-400 mt-0.5 shrink-0" /><span className="text-text-secondary">{s}</span></div>)}
            </div>)}
            {analysis.weaknesses?.length > 0 && (<div className="mt-3 space-y-1">
              {analysis.weaknesses.map((w, i) => <div key={i} className="flex items-start gap-2 text-sm"><span className="w-4 h-4 text-red-400 mt-0.5 shrink-0 text-center">!</span><span className="text-text-secondary">{w}</span></div>)}
            </div>)}
            {analysis.lessonsLearned?.length > 0 && (<div className="mt-4 pt-4 border-t border-border-subtle">
              <h5 className="text-xs uppercase tracking-wider text-text-muted font-semibold mb-3">Lessons</h5>
              {analysis.lessonsLearned.map((l, i) => <div key={i} className="flex items-start gap-2 text-sm text-text-muted mb-1"><ChevronRight className="w-4 h-4 text-accent-gold mt-0.5 shrink-0" /><span>{l}</span></div>)}
            </div>)}
          </div>
        </motion.div>
      )}
