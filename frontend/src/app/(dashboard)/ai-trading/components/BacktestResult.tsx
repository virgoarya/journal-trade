...
export function BacktestResult({ result, analysis, isAnalyzing, onAnalyze, onApplyToPipeline, isApplying }: Props) {
  ...
  const recoveryColor = recoveryFactor === Infinity ? "text-green-400"
    : recoveryFactor >= 3 ? "text-green-400"
    : recoveryFactor >= 1.5 ? "text-yellow-400"
    : "text-red-400";

  const bestMethodology = result.methodologyStats?.length
    ? [...result.methodologyStats].sort((a, b) => b.totalPnL - a.totalPnL)[0]
    : null;
  const worstMethodology = result.methodologyStats?.length
    ? [...result.methodologyStats].sort((a, b) => a.totalPnL - b.totalPnL)[0]
    : null;
  const bestSymbol = result.symbolStats?.length
    ? [...result.symbolStats].sort((a, b) => b.totalPnL - a.totalPnL)[0]
    : null;
  const worstSymbol = result.symbolStats?.length
    ? [...result.symbolStats].sort((a, b) => a.totalPnL - b.totalPnL)[0]
    : null;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {(result.symbols || [result.symbol]).join(", ")} · {result.timeframe} · {result.totalCandles} candles
            </p>
          </div>
        </div>
        {result.backtestId && (
          <div className="text-xs font-mono text-gray-500 bg-gray-950 px-3 py-1.5 rounded-lg border border-gray-800">
            ID: {result.backtestId.substring(0, 8)}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
        ...
        <MetricCard icon={Target} label="Win Rate" value={`${result.winRate}%`}
          color={result.winRate >= 50 ? "text-green-400" : "text-red-400"}
          sub={`${result.winningTrades}W / ${result.losingTrades}L`} />
        <MetricCard icon={isProfitable ? TrendingUp : TrendingDown} label="Net PnL"
          value={`${isProfitable ? "+" : ""}${result.totalPnLPercent}%`}
          color={isProfitable ? "text-green-400" : "text-red-400"}
          sub={`$${result.totalPnL.toFixed(2)}`} />
        <MetricCard icon={TrendingDown} label="Max DD" value={`${result.maxDrawdownPercent}%`}
          color={result.maxDrawdownPercent < 10 ? "text-green-400" : result.maxDrawdownPercent < 20 ? "text-yellow-400" : "text-red-400"} />
      </div>

      {/* Second Row: Recovery Factor + Key Stats */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 relative z-10">
        <MetricCard icon={Shield} label="Recovery Factor"
          value={recoveryFactor === Infinity ? "∞" : recoveryFactor.toFixed(2)}
          color={recoveryColor} />
        <MetricCard icon={Award} label="Profit Factor"
        ...
        <MetricCard icon={Percent} label="Sharpe" value={result.sharpeRatio.toFixed(2)}
          color={result.sharpeRatio >= 1 ? "text-green-400" : "text-gray-300"} />
      </div>
          ...
          <div style={{ height: "200px", width: "100%", position: "relative" }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="equity" stroke="#D4AF37" strokeWidth={2} fill="url(#btEquityGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {/* Advanced Stats: Symbol & Methodology */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10">
                  ...
                  {result.symbolStats.map((stat, i) => (
                    <tr key={i} className="hover:bg-gray-900/30 transition-colors">
                      <td className="px-4 py-3 font-medium text-white flex items-center gap-2">
                        {stat.symbol}
                        {bestSymbol && stat.symbol === bestSymbol.symbol && stat.totalPnL > 0 && (
                          <span className="text-[8px] bg-green-500/20 text-green-400 px-1 rounded">BEST</span>
                        )}
                        {worstSymbol && stat.symbol === worstSymbol.symbol && stat.totalPnL < 0 && (
                          <span className="text-[8px] bg-red-500/20 text-red-400 px-1 rounded">WORST</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3 text-center font-mono text-xs">
                        ...
                        {(stat as any).breakEvenTrades > 0 && (
                          <span className="text-gray-500"> / {(stat as any).breakEvenTrades}BE</span>
                        )}
                      </td>
        ...
        {result.methodologyStats && result.methodologyStats.length > 0 && (
          <div className="space-y-4">
            <h4 className="text-sm font-semibold text-gray-300 uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="bg-gray-950/80 border border-gray-800/80 rounded-xl overflow-hidden">
              <table className="w-full text-left text-sm text-gray-400">
                ...
                <tbody className="divide-y divide-gray-800/50">
                  {result.methodologyStats.map((stat, i) => {
                    const isBest = bestMethodology && stat.methodology === bestMethodology.methodology;
                    const isWorst = worstMethodology && stat.methodology === worstMethodology.methodology;
                    return (
                      <tr key={i} className="hover:bg-gray-900/30 transition-colors">
      ...
      {analysis && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
          ...
          {/* Symbol Insights */}
          {analysis.symbolInsights && analysis.symbolInsights.length > 0 && (
            <div className="mb-4 p-4 bg-gray-950/80 border border-gray-800/80 rounded-xl">
              <p className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-3 flex items-center gap-2">
                <Activity className="w-3 h-3" /> Pair Insights
              </p>
              <div className="space-y-2">
                {analysis.symbolInsights.map((s, i) => {
                  const vColor = s.verdict === "PROFITABLE" ? "text-green-400" : s.verdict === "UNPROFITABLE" ? "text-red-400" : "text-yellow-400";
                  return (
                    <div key={i} className="flex items-start gap-2 text-sm text-gray-300">
                      <span className={`font-semibold shrink-0 ${vColor}`}>{s.verdict}</span>
                      <span className="text-gray-400">{s.symbol} ({s.totalTrades}t, ${s.totalPnL.toFixed(2)}):</span>
          ...
          <div className="p-5 bg-gray-950/80 border border-gray-800/80 rounded-xl">
            ...
            {analysis.recommendedParamChanges && Object.keys(analysis.recommendedParamChanges).length > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-800/50">
                <h5 className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-3">Recommended Changes</h5>
                <pre className="text-xs text-gray-400 bg-gray-900 rounded-lg p-3 overflow-x-auto">
                  {JSON.stringify(analysis.recommendedParamChanges, null, 2)}
                </pre>
              </div>
            )}
          </div>
