...
export function BacktestForm({ onRun, isRunning }: Props) {
  ...
  const addSymbol = (sym: string): boolean => {
    if (atSymbolLimit) return false;
    const s = sym.trim();
    ...
    <div className="glass p-3 space-y-3">
      ...
      <div className="flex items-center justify-between border-b border-accent-gold/15 pb-2">
        <div className="flex items-center gap-2">
          <Settings2 className="w-3.5 h-3.5 text-accent-gold" />
          <h3 className="text-[10px] font-bold text-accent-gold uppercase tracking-widest">Backtest Config</h3>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-bold text-accent-gold-dim uppercase tracking-wider flex items-center gap-1.5">
            <CandlestickChart className="w-3 h-3" />
            Symbols ({selectedSymbols.length}/2)
          </label>
          ...
          {selectedSymbols.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {selectedSymbols.map(sym => (
                <span key={sym} className="inline-flex items-center gap-1 px-2 py-1 bg-accent-gold/20 text-accent-gold text-[10px] rounded font-mono font-bold tracking-wider border border-accent-gold/30">
                  {sym}
                  <button type="button" onClick={() => removeSymbol(sym)} className="hover:text-red-400 ml-0.5">×</button>
                </span>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  if (addSymbol(symbolSearch)) setSymbolSearch("");
                }
              }}
              placeholder={atSymbolLimit ? "Max 2 pairs" : "Search symbol..."}
              disabled={atSymbolLimit}
              className={`flex-1 bg-bg-elevated border border-border-subtle rounded-lg px-2.5 py-2 text-xs text-text-primary focus:border-accent-gold outline-none ${atSymbolLimit ? 'opacity-50 cursor-not-allowed' : ''}`}
            />
          </div>
                ...
                <button
                  ...
                  className={`px-2 py-0.5 text-[9px] font-mono rounded transition ${
                    selectedSymbols.includes(sym)
                      ...
                      : isDisabled
                        ? "bg-black/40 text-gray-700 cursor-not-allowed"
                        : "bg-black/40 text-text-muted hover:text-text-primary border border-border-subtle"
                  }`}
                >
                  {sym}
                </button>
          ...
          <p className="text-[9px] text-gray-600">Max 2 symbols. {atSymbolLimit ? "Remove one first." : "Click or type + Enter."}</p>
        </div>
